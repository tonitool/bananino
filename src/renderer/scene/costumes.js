...
export const isCostumeName = (name) => Object.hasOwn(COSTUMES, name)

/** Geometries and materials are disposed on swap; these add up over a long session. */
const disposeGroup = (group) => {
  group.traverse((child) => {
    if (!child.isMesh) return
    child.geometry?.dispose()
    for (const material of [child.material].flat().filter(Boolean)) material.dispose()
  })
}
